...
void ili_print_info(void)
{
	...
	if (ilits->touch_count == 0)
		...
		input_info(true, ilits->dev,
				"tc:%d mode:%04X lp:%d ED:%d ver:%02d%02d%02d%02d// #%d %d\n",
				ilits->touch_count, ilits->sec_function, ilits->lowpower_mode, ilits->prox_face_mode,
				ilits->fw_cur_info[2], ilits->fw_cur_info[3], ilits->fw_cur_info[4],
...
int ili_sleep_handler(int mode)
{
	...
	case TP_EARLY_SUSPEND:
		...
		if (ilits->prox_face_mode || ilits->gesture) {
			...
			ili_incell_reset_control(ENABLE);
	...
	case TP_EARLY_RESUME:
		...
		if ((ilits->power_status != POWER_OFF_STATUS) && (ilits->power_status != POWER_ON_STATUS)
			...
			if ((ilits->power_status != LP_FACTORY_STATUS) && !ilits->tp_shutdown) {
				ili_incell_reset_control(DISABLE);
				power_disable = true;
				ilits->power_status = POWER_OFF_STATUS;
			}
		}
...
int ili_tddi_init(void)
{
...
#if !IS_ENABLED(CONFIG_SAMSUNG_PRODUCT_SHIP) && IS_ENABLED(CONFIG_QGKI)
	ili_node_init();
#endif
	...
	if (ili_fw_upgrade_handler(NULL) < 0)
				input_err(true, ilits->dev, "%s FW upgrade failed during probe\n", __func__);
#else
	...
	ilits->ws = wakeup_source_register(ilits->sec.fac_dev, "ili_wakelock");
	if (!ilits->ws)
		input_err(true, ilits->dev, "%s wakeup source request failed\n", __func__);
	retval = device_init_wakeup(ilits->sec.fac_dev, true);
	input_info(true, ilits->dev, "%s: device_init_wakeup: fac_dev: ret: %d\n", __func__, retval);
...
//	ili_sec_fn_remove();
	if (ilits->ws) {
		device_init_wakeup(ilits->sec.fac_dev, false);
