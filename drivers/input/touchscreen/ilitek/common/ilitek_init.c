...
void ili_input_register(void)
{
	...
	sec_secure_touch_register(ilits, NULL, 1, &ilits->input->dev.kobj);
...
static int ilitek_vbus_notifier(struct notifier_block *nb, unsigned long cmd, void *data)
{
	...
	case STATUS_VBUS_HIGH:
		...
		ilits->sec_function |= CHARGER_MASK;
		break;
...
static int parse_dt(void)
{
	...
	if (sec_input_check_fw_name_incell(ilits->dev, &ilits->fw_name, NULL) < 0) {
		input_err(true, ilits->dev, "%s: fail to check fw\n", __func__);
		return -EINVAL;
	} else {
