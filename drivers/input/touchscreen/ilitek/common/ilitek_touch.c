...
void ili_report_ap_mode(u8 *buf, int len)
{
	...
	if (ilits->freqIndex != buf[99]) {
		ilits->freqIndex = buf[99];
		input_info(true, ilits->dev, "%s Freq Index : %d\n",
				__func__, ilits->freqIndex);
	}
	if (ilits->strikeFlag != (buf[100] & 0x01)) {
		ilits->strikeFlag = (buf[100] & 0x01);
		input_info(true, ilits->dev, "%s Strike Flag : %d\n",
				__func__, ilits->strikeFlag);
	}
...
#if PROXIMITY_BY_FW
int ili_incell_reset_control(bool onoff)
{
	int ret = 0;
	static bool enabled;
...
int ili_incell_power_control(bool onoff)
{
	int ret = 0;
	static bool enabled;
