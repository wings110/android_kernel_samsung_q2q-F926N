...
#include "M44X_ILI7807S_BS066FBM_panel.h"
...
/* Read vcom reg & vcom mark reg to filter out A14's panel(90Hz)
 * Vcom register 0x1C: the value would be 8X if 120Hz MTP written.
 * Vcom mark register 0xA3: the value would be 10 or 20, 30, 40..
 *             Not zero if 120Hz MTP written
 */
static int ss_read_vcom(struct samsung_display_driver_data *vdd)
{
	u8 recv_buf = 0x00;

	LCD_INFO(vdd, "Read panel Vcom Mark register\n");

	ss_send_cmd(vdd, TX_DDI_VCOM_MARK_PRE);
	ss_panel_data_read(vdd, RX_DDI_VCOM_MARK, &recv_buf, LEVEL_KEY_NONE);

	ss_send_cmd(vdd, TX_MANUFACTURE_ID_READ_PRE); // Page0, Key CLOSE

	LCD_INFO(vdd, "Vcom_mark: 0x%02X\n", recv_buf);

	/* Both should be non-zero 
	 * -> See only Vcom Mark value, cause Vcom value is the same with A14 
	 */
	if (recv_buf)
		return 1;

	/* Not M44 if 1 of them is zero.  */
...
void M44X_ILI7807S_BS066FBM_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_cmd_log_read = ss_read_vcom;
	...
	vdd->debug_data->print_cmds = false; //true;

#if defined(CONFIG_PANEL_BUILTIN_BACKLIGHT)
	ss_blic_s2dps01a01_init();
#endif
}
